...
function initBattery(){
    ...
    navigator.getBattery().then((batt) =>{
        updateBattery = () =>{
            ...
            if(level == 100){ /* Battery is fully charged */
                batteryStatus.innerHTML = `Full battery <i class="ri-battery-2-fill green-color"></i>`
                batteryLiquid.style.height = '103%' /* Slightly overflow to hide the ellipse */
            }
            else if(level <= 20 && !batt.charging){ /* Battery is low and not charging */
                batteryStatus.innerHTML = `Low battery <i class="ri-plug-line animated-red"></i>`
            }
            else if(batt.charging){ /* Battery is currently charging */
                batteryStatus.innerHTML = `Charging... <i class="ri-flashlight-line animated-green"></i>`
            }
            else{ /* Battery is neither low nor charging */
